...
description: Guided, plain-language ON-RAMP for people NEW to SageMaker who want to get started with ML training/inference or LLM fine-tuning/inference on Amazon SageMaker AI or HyperPod. Use for "get started", "train a model", "fine-tune an LLM", "deploy/serve a model", "vibe code on SageMaker", "which instance / will it fit", "SageMaker AI vs HyperPod", or when the user needs the prerequisite setup (IAM, S3, region, quotas) and instance sizing alongside runnable starter code. This is the onboarding/sizing/decision layer for newcomers. (Companion reference skills, if installed, go deeper on the SDK V3 API and the HyperPod `hyp` CLI — defer to them for exact APIs or debugging non-trivial existing code.)
---
